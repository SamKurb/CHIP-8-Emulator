...
{
public:
	AudioPlayer(std::string soundFilePath)
...
private:
    ...
    {
        m_soundEffect = Mix_LoadWAV(m_soundFilePath.data());
        if (m_soundEffect == NULL)
        {
            printf("Failed to load sound effect in AudioPlayer::loadSoundEffect(). SDL_mixer Error: %s\n", Mix_GetError());
            std::exit(1);
        }
    }
    ...
    const std::string m_soundFilePath{};
};